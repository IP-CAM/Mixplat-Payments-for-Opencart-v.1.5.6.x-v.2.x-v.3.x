...
class ControllerPaymentMixplat extends Controller {
    ...
    public function index($payname = 'mixplat') {
        ...
        $this->install();
        $data += $this->load->language('payment/mixplatpro');
        $data += $this->load->language('payment/' . $pname);
        ...
        $data['twostage_show'] = $this->model_payment_mixplat->getTwostage($pname);
        ...
        $data['methodcode'] = $this->model_payment_mixplat->getPaymentType($pname);

        $data['manypoles'] = $this->model_payment_mixplat->getPoles($pname);
        ...
        $data['action'] = $this->url->link('payment/' . $pname, 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('payment/mixplat.tpl', $data));
    ...
    public function capture() {
        ...
        if (isset($this->request->get['capture'])) {
            $this->response->redirect($this->url->link('payment/mixplat/status', 'token=' . $this->session->data['token'], 'SSL'));
        } else {
            $this->response->redirect($this->url->link('payment/mixplat/info', 'order_id=' . (int) $this->request->get['order_id'] . '&token=' . $this->session->data['token'], 'SSL'));
        }
    ...
    public function info() {
        ...
        if (is_array($json)) {
            ...
            //if (isset($json['status']) && $json['status'] == 'waiting') {
                $data['capture']      = $this->url->link('payment/mixplat/capture', 'order_id=' . (int) $this->request->get['order_id'] . '&token=' . $this->session->data['token'], 'SSL');
                $data['cancel']       = $this->url->link('payment/mixplat/cancel', 'order_id=' . (int) $this->request->get['order_id'] . '&token=' . $this->session->data['token'], 'SSL');
