...
class ControllerExtensionPaymentMixplat extends Controller {
    ...
    public function index($payname = 'mixplat') {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate($pname))) {
            $this->model_setting_setting->editSetting('payment_'.$pname, $this->request->post);
        ...
        foreach ($languages as $language) {
            ...
            foreach ($setlangs as $setlang) {
                ...
                if (isset($this->request->post['payment_'.$pname . '_' . $setlang . '_' . $language['language_id']])) {
                    $data['mixplatpro_' . $setlang . '_' . $language['language_id']] = $this->request->post['payment_'.$pname . '_' . $setlang . '_' . $language['language_id']];
                } else {
                    $data['mixplatpro_' . $setlang . '_' . $language['language_id']] = $this->config->get('payment_'.$pname . '_' . $setlang . '_' . $language['language_id']);
                }
            }
            ...
            if ($language['code'] == 'ru-ru') {
                ...
                if (isset($this->request->post[$pname . '_name_' . $language['language_id']])) {
                    $data['mixplatpro_name_' . $language['language_id']] = $this->request->post['payment_'.$pname . '_name_' . $language['language_id']];
                } else if (!$this->config->get('payment_'.$pname . '_name_' . $language['language_id'])) {
                    $data['mixplatpro_name_' . $language['language_id']] = $this->language->get('entry_name_ro');
                } else {
                    $data['mixplatpro_name_' . $language['language_id']] = $this->config->get('payment_'.$pname . '_name_' . $language['language_id']);
                }
            }
            else{
        ...
        $data['pname'] = 'payment_'.$pname;
        $setpros       = $this->model_extension_payment_mixplat->getSettings();

        foreach ($setpros as $setpro) {

            if (isset($this->request->post['payment_'.$pname . '_' . $setpro])) {
                $data['mixplatpro_' . $setpro] = $this->request->post['payment_'.$pname . '_' . $setpro];
            } else {
                $data['mixplatpro_' . $setpro] = $this->config->get('payment_'.$pname . '_' . $setpro);
            }
        }

        if (isset($this->request->post['payment_'.$pname . '_classes'])) {
            $data['mixplatpro_classes'] = $this->request->post['payment_'.$pname . '_classes'];
        } elseif ($this->config->get('payment_'.$pname . '_classes') && isset($this->config->get('payment_'.$pname . '_classes')[0]['payment_'.$pname . '_nalog'])) {
            $data['mixplatpro_classes'] = $this->config->get('payment_'.$pname . '_classes');
        } else {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/' . $pname, 'user_token=' . $this->session->data['user_token'], true),
        );

        $data['action'] = $this->url->link('extension/payment/' . $pname, 'user_token=' . $this->session->data['user_token'], true);
    ...
    public function install() {
        ...
        if (!$this->getEventByCode('mixplat_mail_order_add')){
         
            $code = "mixplat_mail_order_add";
            $trigger = "catalog/model/checkout/order/addOrderHistory/before";
            $action = "extension/payment/mixplat/amail";
    ...
    private function validate($pname) {
        ...
        foreach ($languages as $language) {
            if (empty($this->request->post['payment_'.$pname . '_name_' . $language['language_id']])) {
                $this->error['name_' . $language['language_id']] = $this->language->get('error_name');
            }
        }
        

        if (!$this->request->post['payment_'.$pname . '_project_id']) {
            $this->error['project_id'] = $this->language->get('error_project_id');
        }

        if (!$this->request->post['payment_'.$pname . '_payment_form_id']) {
            $this->error['payment_form_id'] = $this->language->get('error_payment_form_id');
        }

        if (!$this->request->post['payment_'.$pname . '_password']) {
            $this->error['password'] = $this->language->get('error_password');
        }

        if ($this->request->post['payment_'.$pname . '_fixen']) {
            if (!$this->request->post['payment_'.$pname . '_fixen_amount']) {
    ...
    public function status() {
        ...
        $pagination->url   = $this->url->link('extension/payment/mixplat/status', 'user_token=' . $this->session->data['user_token'] . '&page={page}', true);

        $data['pagination'] = $pagination->render();

        $info    = $this->url->link('extension/payment/mixplat/info', 'user_token=' . $this->session->data['user_token'], true);
        $capture = $this->url->link('extension/payment/mixplat/capture', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/payment/mixplat_view_status', $data));
    ...
    private function curlito($data, $rname) {
        ...
        $request = array(
            ...
            'signature'           => md5($payment_id . $this->config->get('payment_'.$order_info['payment_code'] . '_password')),
        );
    ...
    public function capture() {
        ...
        if (isset($this->request->get['capture'])) {
            $this->response->redirect($this->url->link('extension/payment/mixplat/status', 'user_token=' . $this->session->data['user_token'], true));
        } else {
            $this->response->redirect($this->url->link('extension/payment/mixplat/info', 'order_id=' . (int) $this->request->get['order_id'] . '&user_token=' . $this->session->data['user_token'], true));
        }
    ...
    public function info() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/mixplat/status', 'user_token=' . $this->session->data['user_token'], true),
        );

        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_status_info'),
            'href' => $this->url->link('extension/payment/mixplat/info', 'order_id=' . (int) $this->request->get['order_id'] . '&user_token=' . $this->session->data['user_token'], true),
        );
        ...
        $this->response->setOutput($this->load->view('extension/payment/mixplat_info', $data));
